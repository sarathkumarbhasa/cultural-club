...
function EventCard({ event }) {
    ...
    return (
        <motion.div
            ...
            className="glass rounded-none border border-white/5 relative group hover:border-accent-primary/40 overflow-hidden"
        >
            ...
            <div className="p-8">
                <div className="flex justify-between items-start mb-10">
                    <div className="w-12 h-12 flex items-center justify-center bg-white/[0.03] border border-white/10 text-accent-primary group-hover:bg-accent-primary group-hover:text-bg-primary transition-all duration-500">
                        {event.icon}
                    </div>
                ...
                <h3 className="text-2xl font-black mb-4 text-highlight tracking-tight group-hover:text-accent-primary transition-colors">
                    {event.name}
                </h3>
                ...
                <div className="flex flex-col gap-4">
                    <button
                        ...
                        className="flex items-center justify-between text-[10px] font-black uppercase tracking-[0.3em] text-accent-primary group-hover:text-highlight transition-colors"
                    >
                    ...
                    <a
                        href="#register"
                        className="w-full py-4 text-center bg-transparent border border-white/10 text-[10px] font-black uppercase tracking-[0.4em] group-hover:border-accent-primary hover:bg-white/5 transition-all"
                    >
                        Initialize
                    </a>
                </div>
...
export default function Events() {
    ...
    return (
        <section id="events" className="py-32 px-6 bg-bg-primary">
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row justify-between items-end mb-24 gap-12">
                    ...
                    <div className="flex p-1 bg-white/[0.02] border border-white/5">
                        {["technical", "nonTechnical"].map((tab) => (
                            <button
                                key={tab}
                                onClick={() => setActiveTab(tab)}
                                className={`px-10 py-3 text-[10px] font-black uppercase tracking-[0.3em] transition-all ${activeTab === tab
